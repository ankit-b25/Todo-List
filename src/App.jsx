    ...
    <>
      ...
      <div className='mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[85vh] md:w-[40%]'>
        ...
        <div className="todos">
          ...
          {todos.map(item => {
            return (
              (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex justify-between items-center my-3">
                
                <div className='flex gap-5'>
                  <input name={item.id} onClick={handleCheckbox} type="checkbox" checked={item.isCompleted} id='' />
                  <div className={`${item.isCompleted ? "line-through" : ""} break-all pr-1`}>{item.todo}</div>
                </div>
